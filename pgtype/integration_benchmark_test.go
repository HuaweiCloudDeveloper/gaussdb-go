...
func BenchmarkQueryTextFormatDecode_PG_int4_to_Go_int16_1_rows_1_columns(b *testing.B) {
	defaultConnTestRunner.RunTest(context.Background(), b, func(ctx context.Context, _ testing.TB, conn *gaussdb.Conn) {
		b.ResetTimer()
		var v [1]int16
		for i := 0; i < b.N; i++ {
			rows, _ := conn.Query(
				...
				gaussdb.QueryResultFormats{gaussdb.TextFormatCode},
			)
			_, err := gaussdb.ForEachRow(rows, []any{&v[0]}, func() error { return nil })
			if err != nil {
				b.Fatal(err)
			}
		}
	...
	defaultConnTestRunner.RunTest(context.Background(), b, func(ctx context.Context, _ testing.TB, conn *gaussdb.Conn) {
		...
		for i := 0; i < b.N; i++ {
			rows, _ := conn.Query(
				ctx,
				`select n::int4 + 0 from generate_series(1, 1) n`,
				gaussdb.QueryResultFormats{gaussdb.BinaryFormatCode},
			)
			_, err := gaussdb.ForEachRow(rows, []any{&v[0]}, func() error { return nil })
			if err != nil {
				b.Fatal(err)
...
func BenchmarkQueryTextFormatDecode_PG_int4_to_Go_int16_1_rows_10_columns(b *testing.B) {
	defaultConnTestRunner.RunTest(context.Background(), b, func(ctx context.Context, _ testing.TB, conn *gaussdb.Conn) {
		...
		for i := 0; i < b.N; i++ {
			...
			_, err := gaussdb.ForEachRow(rows, []any{&v[0], &v[1], &v[2], &v[3], &v[4], &v[5], &v[6], &v[7], &v[8], &v[9]}, func() error { return nil })
			if err != nil {
				b.Fatal(err)
			}
		}
...
func BenchmarkQueryTextFormatDecode_PG_Int4Array_With_Go_Int4Array_10(b *testing.B) {
	defaultConnTestRunner.RunTest(context.Background(), b, func(ctx context.Context, _ testing.TB, conn *gaussdb.Conn) {
		...
		for i := 0; i < b.N; i++ {
			...
			_, err := gaussdb.ForEachRow(rows, []any{&v}, func() error { return nil })
			if err != nil {
				b.Fatal(err)
			}
		}
