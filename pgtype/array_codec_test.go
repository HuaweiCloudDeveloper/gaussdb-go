...
func TestArrayCodecFlatArrayString(t *testing.T) {
	...
	queryModes := []gaussdb.QueryExecMode{gaussdb.QueryExecModeSimpleProtocol, gaussdb.QueryExecModeDescribeExec}
...
func TestArrayCodecArray(t *testing.T) {
	...
	ctr.AfterConnect = func(ctx context.Context, t testing.TB, conn *gaussdb.Conn) {
		gaussdbtest.SkipCockroachDB(t, conn, "Server does not support multi-dimensional arrays")
	}

	ctr.RunTest(context.Background(), t, func(ctx context.Context, t testing.TB, conn *gaussdb.Conn) {
		for i, tt := range []struct {
			expected any
		}{
			{pgtype.Array[int32]{
