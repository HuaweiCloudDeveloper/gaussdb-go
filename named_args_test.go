...
func TestNamedArgsRewriteQuery(t *testing.T) {
	...
	for i, tt := range []struct {
		...
		},
		{
			...
			namedArgs:    gaussdb.NamedArgs{"extra": int32(1)},
...
func TestStrictNamedArgsRewriteQuery(t *testing.T) {
	...
	for i, tt := range []struct {
		sql             string
		namedArgs       gaussdb.StrictNamedArgs
		expectedSQL     string
		expectedArgs    []any
		...
		{
			sql:             "@all @matches",
			namedArgs:       gaussdb.StrictNamedArgs{"all": int32(1), "matches": int32(2)},
			expectedSQL:     "$1 $2",
		...
		{
			sql:             "extra provided argument",
			namedArgs:       gaussdb.StrictNamedArgs{"extra": int32(1)},
